...
% stimuli corpus using the single realization version of the auditory
% periphery model.
%
%
% Author:  Michael R. Wirtzfeld ( michael.wirtzfeld [at] gmail.com )
% Modification Date:  Thursday, February 19, 2015 (copy of "ph_mult_rep.m")
...
parameters = processing_parameters();
    ...
    parameters.model.number_of_realizations = 1;
...
target_directory=[ 'rep_1_Case_' num2str(columns_to_process) '_Words_' num2str(start_index) ...
...
for chimaera_index = CASES_TO_PROCESS
    ...
    for case_index = word_set
        ...
        fprintf(1, '\nChimaera: %d,  Row: %d  (%d To Process)\n', chimaera_index, case_index, numel(word_set));
        ...
        %
        original_neurograms = sentence_processing_1_rep( x(:), parameters, ...
        ...
        %
        chimaera_neurograms = sentence_processing_1_rep( added_filter_bands(:), parameters, ...
            working_chimaera_data_set(case_index, chimaera_index).placement_index, ...
            working_chimaera_data_set(case_index, chimaera_index).length );
        
        
        save_raw_neurograms( original_neurograms, 'original', chimaera_index, case_index, ...
            working_chimaera_data_set(case_index, chimaera_index).word_string, 'single_rep', parameters );
        %
...
%             working_chimaera_data_set( case_index, chimaera_index ).alsr = alsr;
    ...
    filename_modifier = [ '_single_rep' parameters.model.model_version ];
...
%   -Copy of "ph_mult_rep.m", which was renamed from "processing_harness.m".
